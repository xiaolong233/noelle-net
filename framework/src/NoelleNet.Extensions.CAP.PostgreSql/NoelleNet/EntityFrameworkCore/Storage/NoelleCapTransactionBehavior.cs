...
public class NoelleCapTransactionBehavior<TRequest, TResponse> : IPipelineBehavior<TRequest, TResponse?> where TRequest : notnull
{
    private readonly ILogger<NoelleCapTransactionBehavior<TRequest, TResponse>> _logger;
    private readonly DbContext _dbContext;
    private readonly ICapPublisher _capPublisher;

    /// <summary>
    /// 创建一个新的 <see cref="NoelleCapTransactionBehavior{TRequest, TResponse}"/> 实例
    /// </summary>
    /// <param name="logger">日志记录器</param>
    /// <param name="dbContext">数据库上下文实例</param>
    /// <param name="capPublisher">分布式事件总线</param>
    /// <exception cref="ArgumentNullException"></exception>
    public NoelleCapTransactionBehavior(ILogger<NoelleCapTransactionBehavior<TRequest, TResponse>> logger, DbContext dbContext, ICapPublisher capPublisher)
    {
