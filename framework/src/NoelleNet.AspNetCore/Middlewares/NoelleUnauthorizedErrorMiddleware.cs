...
public class NoelleUnauthorizedErrorMiddleware(RequestDelegate next)
{
    ...
    public async Task InvokeAsync(HttpContext context)
    {
        ...
        if (context.Response.StatusCode == StatusCodes.Status401Unauthorized)
        {
            ...
            await context.Response.WriteAsJsonAsync(new NoelleErrorResponseDto(new NoelleErrorDetailDto(localizer[NoelleConstants.ErrorCodes.Unauthorized]) { Code = NoelleConstants.ErrorCodes.Unauthorized }));
        }
    }
}
