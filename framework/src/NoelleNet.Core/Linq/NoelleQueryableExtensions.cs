...
public static class NoelleQueryableExtensions
{
    ...
    {
        return WhereIf(source, condition, Dynamic.Core.ParsingConfig.Default, predicate, args);
    }
    ...
    /// <param name="args">表达式的参数</param>
    /// <returns></returns>
    public static IQueryable<T> WhereIf<T>(this IQueryable<T> source, bool condition, Dynamic.Core.ParsingConfig config, string predicate, params object?[] args)
    {
        ArgumentNullException.ThrowIfNull(source);

        return condition ? Dynamic.Core.DynamicQueryableExtensions.Where<T>(source, config, predicate, args) : source;
    }
    ...
    public static IQueryable<T> OrderByIf<T>(this IQueryable<T> source, bool condition, string? sort)
    {
        ...
        return condition ? Dynamic.Core.DynamicQueryableExtensions.OrderBy(source, sort ?? string.Empty) : source;
    }
}
