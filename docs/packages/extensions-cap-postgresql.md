# NoelleNet.Extensions.CAP.PostgreSql

支持 CAP 事务发件箱的 `ITransactionManager` 实现（PostgreSQL）✧

```bash
dotnet add package NoelleNet.Extensions.CAP.PostgreSql
```

⚠️ **命名空间有坑**：类名是 `NoelleCapTransactionManager`，但命名空间是 **`NoelleNet.Uow`**（不是 `NoelleNet.Extensions.CAP.PostgreSql`）。因为三个数据库包的类名完全相同、命名空间也相同，**同一个项目里只能引用其中一个**，否则类型冲突。

依赖 `DotNetCore.CAP.PostgreSql` 10.0.2 与 [NoelleNet.EntityFrameworkCore](entity-framework-core.md)。
...
⚠️ 发件箱表由 CAP 维护，首次使用前需按 CAP 的方式初始化（执行建表脚本或调用其初始化 API）。PostgreSQL 下注意 schema 归属：发件箱表与业务表若不在同一个 schema / 数据库，事务无法覆盖消息表。
...
| MySQL | [NoelleNet.Extensions.CAP.MySql](extensions-cap-mysql.md) | `DotNetCore.CAP.MySql` |
